...
import pickle
import imutils  # Nueva importación para manejar contornos y rotaciones
import random
...
def plot_button_pushed(root):
    ...
    # Rotación automática para alinear la placa horizontalmente
    # Detectar bordes y calcular el ángulo de inclinación
    edges = cv2.Canny(img_resized, 50, 150, apertureSize=3)
    ...
    # Rotar la imagen para alinear
    rotated = imutils.rotate_bound(img_resized, angle)
    plt.figure()
    plt.imshow(rotated, cmap='gray')
    plt.title(f"Imagen Rotada en {angle:.2f} grados")
    plt.axis('off')

    # Binarizar la imagen usando el umbral adaptativo
    binary_image = cv2.adaptiveThreshold(rotated, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, 
    ...
    if w > 2000:
        temp = remove_small_objects(binary_image > 0, min_size=3500)
    else:
        temp = remove_small_objects(binary_image > 0, min_size=3000)
    ...
    temp2 = (binary_image > 0) & ~temp
    temp2 = remove_small_objects(temp2, min_size=250)

    # Detección de bordes usando el operador de Sobel
    edges_sobel = sobel(temp2)
    edges_binary = edges_sobel > 0.1  # Umbral para binarizar los bordes
    ...
    # Dibujar cuadros delimitadores en la imagen original rotada
    fig, ax = plt.subplots()
    ...
    for prop in props:
        minr, minc, maxr, maxc = prop.bbox
        char_img = temp2[minr:maxr, minc:maxc]
        if char_img.size == 0:
            continue
